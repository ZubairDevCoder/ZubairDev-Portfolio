...
function ExperienceItem({ exp, idx, start, end, scrollYProgress, layout }) {
  ...
  return (
    ...
    >
      <motion.div
        className="flex-shrink-0 mt-2 w-6 h-6 rounded-full bg-white shadow-[0_0_0_6px_rgba(255,255,255,0.1)]"
        style={{ scale: markerScale, opacity: markerOpacity }}
      />
      <motion.article
        ...
        <div className="sticky top-0 h-screen flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center px-6 pb-10">
            ...
            <div className="relative w-full max-w-md md:hidden ">
              <div className="absolute left-3 top-0 bottom-0 w-[6px] bg-white/15 rounded" />
              <div className="relative flex flex-col gap-10 mt-6 pb-20">
                {experiences.map((exp, idx) => {
                  const start = idx === 0 ? 0 : thresholds[idx - 1];
                  const end = thresholds[idx];
                  return (
